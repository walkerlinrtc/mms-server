...
class M4sMediaSink : public MediaSink, public ObjTracker<M4sMediaSink> {
public:
    M4sMediaSink(ThreadWorker *worker);
    virtual ~M4sMediaSink();
    bool init();
    bool recv_combined_init_segment(std::shared_ptr<Mp4Segment> mp4_seg);
    ...
    void set_combined_init_segment_cb(const std::function<boost::asio::awaitable<bool>(std::shared_ptr<Mp4Segment> msg)> & cb);
    ...
    void close() override;
private:
    std::function<boost::asio::awaitable<bool>(std::shared_ptr<Mp4Segment> seg)> combined_init_seg_cb_;

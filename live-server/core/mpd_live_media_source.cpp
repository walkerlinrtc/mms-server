...
using namespace mms;
MpdLiveMediaSource::MpdLiveMediaSource(ThreadWorker *worker, std::weak_ptr<StreamSession> session, std::shared_ptr<PublishApp> app) : MediaSource("mpd", session, app, worker) {
    availabilityStartTime = Utils::get_utc_time();
}
...
bool MpdLiveMediaSource::on_audio_init_segment(std::shared_ptr<Mp4Segment> seg) {
    ...
    seg->set_filename(stream_name_ + "/audio-init.mp4");
    audio_init_seg_ = seg;
    // update_mpd();
    return true;
}
...
bool MpdLiveMediaSource::on_video_init_segment(std::shared_ptr<Mp4Segment> seg) {
    std::unique_lock<std::shared_mutex> lck(segments_mtx_);
    seg->set_filename(stream_name_ + "/video-init.mp4");
    video_init_seg_ = seg;
    // update_mpd();
    return true;
}
...
void MpdLiveMediaSource::update_mpd() {
    ...
    if (video_segments_.size() > 0) {
        last_duration = std::max((double)video_segments_[video_segments_.size()-1]->get_duration(), last_duration);
    }
    ...
    if (audio_init_seg_ && !audio_segments_.empty()) {
        ...
        for (size_t i = start_seg_index; i < audio_segments_.size(); ++i) {
            ss << "                        <S t=\"" << audio_segments_.at(i)->get_start_timestamp() << "\" "
                                          << "d=\"" << audio_segments_.at(i)->get_duration() << "\" />"  << std::endl;
        }
        ss << "                    </SegmentTimeline>" << std::endl;
    ...
    if (video_init_seg_ && !video_segments_.empty()) {
        ...
        ss << "            <Representation id=\"video\" bandwidth=\"800000\" codecs=\"avc1.64001e\">" << std::endl;
        ss << "                <SegmentTemplate initialization=\"$RepresentationID$-init.m4s\" "
                                            << "media=\"$RepresentationID$-$Number$.m4s\" "
        ...
        for (size_t i = start_seg_index; i < video_segments_.size(); ++i) {
            ss << "                        <S t=\"" << video_segments_.at(i)->get_start_timestamp() << "\" "
                                          << "d=\"" << video_segments_.at(i)->get_duration() << "\" />"  << std::endl;
        }
        ss << "                    </SegmentTimeline>" << std::endl;
        ss << "                </SegmentTemplate>" << std::endl;
        ss << "            </Representation>" << std::endl;
...
std::shared_ptr<MediaBridge> MpdLiveMediaSource::get_or_create_bridge(const std::string & id, std::shared_ptr<PublishApp> app, const std::string & stream_name) {
    ((void)id);
    ((void)app);
    ((void)stream_name);
    return nullptr;

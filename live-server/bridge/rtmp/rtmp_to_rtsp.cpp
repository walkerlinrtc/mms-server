...
bool RtmpToRtsp::on_metadata(std::shared_ptr<RtmpMessage> metadata_pkt) {
    ...
    auto amf0_data = metadata_->get_amf0_data();
    if (has_video_) {
        ...
        if (video_codec_id == VideoTagHeader::AVC) {
            ...
            video_rtp_packer_ = std::make_shared<RtpPacker>();
            ...
            video_rtp_packer_ = std::make_shared<H265RtpPacker>();
        } else {
            return false;
        }
    }
...
boost::asio::awaitable<bool> RtmpToRtsp::on_video_packet(std::shared_ptr<RtmpMessage> video_pkt) {
    if (!video_codec_) {
        spdlog::info("unknown video codec");
        co_return false;
    }
...
boost::asio::awaitable<bool> RtmpToRtsp::on_audio_packet(std::shared_ptr<RtmpMessage> audio_pkt) {
    if (!audio_codec_) {
        co_return false;
    }
...
boost::asio::awaitable<bool> RtmpToRtsp::process_h264_packet(std::shared_ptr<RtmpMessage> video_pkt) {
    ...
    int64_t pts = video_pkt->timestamp_ + header.composition_time;
    ...
    if (!got_video_key_frame_) {
        if (header.is_key_frame()) {
            got_video_key_frame_ = true;
        }
    }
...
bool RtmpToRtsp::generate_sdp() {
    ...
    CORE_DEBUG("generate sdp:{}", sdp_->to_string());
    return true;
}

void RtmpToRtsp::close() {

...
#include <iostream>
namespace mms {
    struct stun_finger_print_attr : public stun_msg_attr
    {
        stun_finger_print_attr(uint8_t *data, size_t len) : stun_msg_attr(STUN_ATTR_FINGERPRINT)
        {
        ...
        size_t size()
        {
            return stun_msg_attr::size() + sizeof(uint32_t);
        }
        ...
        int32_t encode(uint8_t *data, size_t len)
        {
            ...
            int32_t consumed = stun_msg_attr::encode(data, len);
            if (consumed < 0)
            {
                return -1;
            }
        ...
        {
            uint8_t *data_start = data;
            int32_t consumed = stun_msg_attr::decode(data, len);
            if (consumed < 0)
            {
                return -1;
            }
